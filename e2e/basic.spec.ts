  ...
  test("should navigate to pricing page", async ({ page }) => {
    ...
    await expect(
      page
        ...
        .or(page.getByText(/pricing/i).first()),
    ).toBeVisible();
  });
...
test.describe("Authentication Flow", () => {
  test("should redirect unauthenticated users from dashboard", async ({
    ...
    await expect(page)
      .not.toHaveURL(/.*dashboard.*/, { timeout: 5000 })
      .catch(() => {
  ...
  test("should redirect unauthenticated users from settings", async ({
    ...
    await expect(page)
      .not.toHaveURL(/.*settings.*/, { timeout: 5000 })
      .catch(() => {
        return expect(page.getByText(/sign in/i).first()).toBeVisible();
      });
  });
  ...
  test("sign in button should trigger GitHub OAuth", async ({ page }) => {
    ...
    if (popup) {
      ...
      await page
        .waitForURL(/github\.com|\/api\/auth/, { timeout: 5000 })
        .catch(() => {
...
test.describe("API Validation", () => {
  ...
  test("publish endpoint should require authentication", async ({
    request,
  }) => {
    const response = await request.post("/api/posts/publish", {
      data: {
