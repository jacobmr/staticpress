...
export function DashboardClient({
    ...
    const fetchRemainingPosts = async () => {
      try {
        const response = await fetch(
          `/api/posts?offset=${initialPosts.length}`,
        );
        if (response.ok) {
          ...
          setPosts((prev) => {
            ...
            const newPosts = data.posts.filter(
              (p: HugoPost) => !existingPaths.has(p.path),
            );
  ...
  const handleSelectPost = (post: HugoPost) => {
    ...
    console.log(
      "[SelectPost] Raw markdown from GitHub:",
      post.content.substring(0, 500),
    );
    const htmlContent = marked.parse(post.content) as string;
    console.log(
      "[SelectPost] After marked.parse():",
  ...
  const handlePublish = async () => {
    ...
    if (processedContent.includes("data:image")) {
      setSaveMessage(
        "Error: Some images failed to upload. Please remove and re-add them.",
      );
      setSaveStatus("error");
    ...
    try {
      console.log(
        "[Publish] Content being sent (first 500 chars):",
