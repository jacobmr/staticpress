...
export async function POST(request: Request) {
  try {
    ...
    try {
      const workflowContents = await github.getRepoContents(
        owner,
        repo,
      ...
      if (
        ...
        !Array.isArray(workflowContents[0]) &&
        "sha" in workflowContents[0]
      ) {
        const currentWorkflow = await github.getFileContent(
          owner,
          repo,
          ".github/workflows/hugo.yml",
        );
    ...
    try {
      ...
      return NextResponse.json(
        {
          error:
            "Failed to set up theme. The theme may require manual configuration.",
        },
        { status: 500 },
      );
    }
    ...
    const { getUserByGithubId, logEvent, upsertUserRepository } =
      await import("@/lib/db");
