...
export async function POST(request: Request) {
  try {
    ...
    const { blogName, description, isPrivate, engine, theme } =
      await request.json();
    ...
    try {
      ...
      console.log(
        `[Create] Repository created from ${selectedEngine} template: ${owner}/${repoName}`,
      );
    } catch (error) {
      const errorMessage =
...
`;
          ...
          console.log(
            `[Create] Customized hugo.toml with blog name: ${blogName}, theme: ${themeId}`,
          );
        }
      } catch (error) {
        // Non-fatal - blog will work with default title
        console.log(
          `[Create] Could not customize hugo.toml: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
      ...
      if (themeId && themeInfo) {
        try {
          console.log(
            `[Create] Setting up theme ${themeId} from ${themeInfo.repo}`,
          );
          ...
          console.log(
            `[Create] Could not set up theme submodule: ${error instanceof Error ? error.message : "Unknown error"}`,
          );
        }
      }
    } else {
...
`;
          ...
          console.log(
            `[Create] Customized config.yaml with blog name: ${blogName}`,
          );
        }
      } catch (error) {
        // Non-fatal - blog will work with default title
        console.log(
          `[Create] Could not customize config.yaml: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    }

    // Get or create user in database and save repo config
    const { getUserByGithubId, upsertUserRepository, logEvent } =
      await import("@/lib/db");
