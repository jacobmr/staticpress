...
export async function POST() {
  try {
    ...
    const results = await Promise.all(
      configCandidates.map((name) =>
        github.getFile(repoConfig.owner, repoConfig.repo, name),
      ),
    );

    let configFilename = "";
    let configFile: (typeof results)[number] = null;
    for (let i = 0; i < results.length; i++) {
      if (results[i]) {
        configFile = results[i];
        configFilename = configCandidates[i];
        break;
      }
    }
...
`;
    ...
    if (
      hugoConfigContent.match(
        /baseURL\s*=\s*["']https?:\/\/example\.org\/?["']/,
      )
    ) {
      const isUserSite =
        repoConfig.repo.toLowerCase() ===
        `${repoConfig.owner.toLowerCase()}.github.io`;
...
<img src="{{ $u }}" alt="{{ .Text }}"{{ with .Title }} title="{{ . }}"{{ end }} loading="lazy" />`;
    ...
    const hookFile = await github.getFile(
      repoConfig.owner,
      repoConfig.repo,
      renderHookPath,
    );
