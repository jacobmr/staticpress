...
export async function GET(
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        getErrorRedirectUrl(errorDescription || error),
      );
    }
    ...
    if (!code || !state) {
      return NextResponse.redirect(
        getErrorRedirectUrl("Missing authorization code or state"),
      );
    }

    // Dynamically import database functions
    const { getUserByGithubId, getSupabaseClient, logEvent } =
      await import("@/lib/db");
    ...
    if (stateError || !storedState) {
      return NextResponse.redirect(
        getErrorRedirectUrl("Invalid state parameter"),
      );
    }
    ...
    if (new Date(storedState.expires_at) < new Date()) {
      ...
      return NextResponse.redirect(
        getErrorRedirectUrl("Authorization expired, please try again"),
      );
    }
    ...
    if (!provider.exchangeCodeForToken) {
      return NextResponse.redirect(
        getErrorRedirectUrl("Platform does not support OAuth flow"),
      );
    }
    ...
    try {
      ...
      return NextResponse.redirect(
        getErrorRedirectUrl("Failed to exchange authorization code"),
      );
    }
    ...
    if (existingPlatform) {
      ...
      if (updateError) {
        ...
        return NextResponse.redirect(
          getErrorRedirectUrl("Failed to update platform credentials"),
        );
      }
    } else {
      // Insert new platform
      ...
      if (insertError) {
        ...
        return NextResponse.redirect(
          getErrorRedirectUrl("Failed to store platform credentials"),
        );
      }
    }
      ...
      try {
        ...
        if (repoDetails) {
          ...
          if (existingProject) {
            ...
            await supabase.from("deployment_projects").insert({
          ...
          return NextResponse.redirect(
            getSetupSuccessRedirectUrl(validPlatform, project.productionUrl),
          );
        }
      } catch (setupError) {
...
function getSetupSuccessRedirectUrl(
  platform: string,
