...
export async function GET(
  ...
  try {
    ...
    // Verify ownership
    const repositories = project.repositories as unknown as {
      user_id: number;
      owner: string;
      repo: string;
    }[];
    ...
    if (platformData) {
      try {
        ...
        const provider = await getDeploymentProvider(
          project.platform as DeploymentPlatform,
        );
        ...
        providerProject = await provider.getProject(
          credentials,
          project.project_id,
        );
      } catch {
    ...
    return NextResponse.json({
      project: {
        ...
        customDomains:
          providerProject?.customDomains || project.custom_domains || [],
