...
export class CloudflareProvider implements DeploymentProvider {
  ...
  private async getAccountIdFromCredentials(
    ...
    try {
      ...
      if (!response.success || !response.result) {
        return {
          success: false,
          error:
            response.errors?.[0]?.message || "Failed to trigger deployment",
        };
      }
    ...
    try {
      ...
      if (!response.success || !response.result) {
        return {
          status: "failed",
          error:
            response.errors?.[0]?.message || "Failed to get deployment status",
        };
      }
      ...
      return {
        ...
        error:
          status === "failed"
            ? `Build failed at stage: ${deployment.latest_stage.name}`
            : undefined,
      };
    } catch (error) {
      return {
    ...
    try {
      ...
      for (const stage of deployment.stages) {
        ...
        if (stage.ended_on) {
          ...
          const duration =
            (new Date(stage.ended_on).getTime() -
              new Date(stage.started_on).getTime()) /
            1000;
          logs.push(
            `[${endTime}] Stage ${stage.name} completed in ${duration}s`,
          );
        }
      }
      ...
      if (deployment.deployment_trigger?.metadata) {
        ...
        logs.unshift(
          `Commit: ${meta.commit_hash?.substring(0, 7) || "N/A"} - ${meta.commit_message || "No message"}`,
        );
    ...
    try {
      ...
      if (!response.success || !response.result) {
        return {
          success: false,
          error:
            response.errors?.[0]?.message || "Failed to rollback deployment",
        };
      }
