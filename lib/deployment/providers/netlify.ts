...
export class NetlifyProvider implements DeploymentProvider {
  ...
  async getDeploymentStatus(
    ...
    return {
      ...
      completedAt: deploy.published_at
        ? new Date(deploy.published_at)
        : undefined,
  ...
  async setCustomDomain(
    ...
    try {
      ...
      const isDomainConfigured =
        site.domain_aliases.includes(domain) || site.custom_domain === domain;
  ...
  async deleteProject(
    ...
    try {
      await this.makeRequest(`/sites/${projectId}`, credentials.accessToken, {
        method: "DELETE",
      });
      return true;
    } catch {
  ...
  async exchangeCodeForToken(
    ...
    if (!clientId || !clientSecret) {
      throw new Error(
        "NETLIFY_CLIENT_ID and NETLIFY_CLIENT_SECRET environment variables are required",
      );
    }
  ...
  async autoSetupProject(
    ...
    try {
      ...
      while (existingSite) {
        ...
        if (
          existingSite.build_settings?.repo_url?.includes(
            `${owner}/${repoName}`,
          )
        ) {
